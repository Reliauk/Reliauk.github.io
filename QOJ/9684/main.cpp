...
bool Leq(int al, int ar, int at, int bl, int br, int bt) { return true; }
...
bool Check(int ml, int mr, int mt) {
  ...
  for (int t = 1; t <= n; ++t) {
    for (int r = std::max(1, t - A + 1); r <= t; ++r)
      dp[r] = std::min(dp[r], t + Query(mp[r][t - r]));
    for (int r = std::max(1, t - A + 1); r <= t; ++r) Cmin(r, dp[r] - r);
  }
  return dp[n] <= k;
}
