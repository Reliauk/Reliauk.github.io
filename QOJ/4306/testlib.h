...
struct InStream {
  ...
  void readSecret(
      std::string secret, TResult mismatchResult = _wa,
...
NORETURN void InStream::quit(TResult result, const char *msg) {
  ...
  if (resultName != "") {
    ...
    if (appesMode) {
      ...
      std::fprintf(resultFile, "%s %s", errorName.c_str(),
...
void InStream::readSecret(std::string secret, TResult mismatchResult,
                          std::string mismatchMessage, std::string eofMessage) {
  if (seekEof()) quits(mismatchResult, eofMessage);
  if (readWord() != secret) quits(mismatchResult, mismatchMessage);
  eoln();
}
...
void readBothSecrets(std::string secret) {
  ...
  ouf.readSecret(secret, _wa, "Possible tampering with the output",
...
NORETURN void quit(TResult result) { ouf.quit(result, ""); }
...
void compareTokens(int lineNo, std::string a, std::string b,
                   char separator = ' ') {
  ...
  if (toka.size() != tokb.size())
    quitf(_wa, "%sexpected: %d tokens, found %d tokens", dif.c_str(),
          int(toka.size()), int(tokb.size()));
  for (int i = 0; i < int(toka.size()); i++)
    if (toka[i] != tokb[i])
      quitf(_wa, "%son the %s token, expected: '%s', found: '%s'", dif.c_str(),
            englishTh(i + 1).c_str(), compress(toka[i]).c_str(),
            compress(tokb[i]).c_str());
